...
static map_node
newNode (const char* description, const char* name) {
...
static player
newPlayer (map_node initial_pos) {
...
static void
linkNodes (map_node node1, map_node node2, int direction)
{
...
// player API
void
MovePlayer (player p, int direction)
{
...
char*
DescribePlayerPos (struct player p)
{
...
player InitGame()
{
	...
	linkNodes(map->ajacent_nodes[NORTH],
			newNode("another empty room og", "room three"), WEST);

	DPRINT("\tinitializing player...");
	player player = newPlayer(map);

	AddItem(map->item_list, "lamp",
			(struct item) {
			.name="lamp",
			.map_description="a lamp"
			}
	       );
